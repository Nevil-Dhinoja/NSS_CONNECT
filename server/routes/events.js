...
router.get('/reports/:id/download', verifyToken, (req, res) => {
  ...
  db.query(reportSql, [id], (err, reportResults) => {
    ...
    // Fix for cross-platform path issues
    let safeFilePath = report.file_path.replace(/\\/g, '/');
    const absolutePath = path.resolve(__dirname, '..', safeFilePath);
    console.log('Trying to download:', absolutePath);
